...
pub fn part2() {
    ...
    let mut i = 4006100000;

    let res = loop {
        if i % 100000 == 0 {
            println!("Testing: {}", i);
        }
        let mut test_r = r.clone();
        test_r.a = i;
        let output = run_program(&program, &mut test_r);
        let is_done = program.len() == output.len()
            && zip(program.iter(), output.iter()).all(|(x, y)| x == y);
        if is_done {
            break i;
        }
        i += 1;
    };
    println!("Register A: {}", res);
}

fn run_program(program: &Vec<u8>, r: &mut Registry) -> Vec<u8> {
    let mut i: usize = 0;
